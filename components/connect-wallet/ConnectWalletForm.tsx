...
export default function ConnectWalletForm() {
  ...
  const socialButtons = [
    { id: 'facebook', name: 'Facebook', icon: '/icons/facbook.png', bgColor: '#1877F2' },
    { id: 'google', name: 'Google', icon: '/icons/google.png', bgColor: '#FFFFFF' },
    { id: 'apple', name: 'Apple', icon: '/icons/apple.png', bgColor: '#000000' },
  ];

  const walletOptions = [
    { id: 'metamask', name: 'MetaMask', icon: '/connect-wallet/metamask.png' },
    { id: 'phantom', name: 'Phantom', icon: '/connect-wallet/phantom.png' },
    { id: 'coinbase', name: 'Coinbase Smart Wallet', icon: '/connect-wallet/coin-base.png' },
    ...
    >
      ...
      {isConnected ? (
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-green-500/20 border border-green-500/50">
            ...
            <p className="text-green-400 font-mono text-xs break-all">{address}</p>
          </div>
          <button
            onClick={() => disconnect()}
        ...
        <>
          <div className="mb-6">
            <p className="text-white/80 text-sm mb-4 text-center">Sign Up With:</p>
              ...
              >
                ...
                {wallet.icon ? (
                  ...
                  <div className="w-6 h-6 rounded-full bg-[#36E8CA] flex items-center justify-center">
                    <span className="text-xs font-bold text-white">{wallet.count}</span>
                  </div>
                )}
              </button>
            ))}
      ...
      <div className="mt-6 pt-6 border-t border-white/10 text-center">
        <p className="text-white/60 text-xs">
          Powered by <span className="font-bold text-white">THEROWD</span>
        </p>
      </div>
    </div>
  );
